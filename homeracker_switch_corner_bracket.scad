// HomeRacker switch corner bracket - a U-channel that slides along a
// HomeRacker support (used here as a horizontal rail) and locks in place
// with the support's own lock pins. The channel's cavity runs its full
// length, uninterrupted, so the rail passes freely through. An L-shaped
// bracket hangs off the bottom of the U - off the outer face of the
// floor, the flat side with no pin holes, at one end, dropping down past
// the floor (negative Z, in this same orientation). The L shape is in
// plan view (look straight down the Z axis to see it): one arm runs back
// along the rail, the other bends 90 degrees and runs sideways, so the
// corner of the L catches the corner of the switch. Print FOUR of these:
// two on one rail catch one edge of the switch, two on a second,
// parallel rail catch the other edge.
//
